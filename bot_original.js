(function () {
    'use strict';
    console.log("🚀 GrepoBot V11.80 Cargado desde el servidor seguro.");
    ...
    const MITICAS = {
        ...
        'cyclops': 'Cíclope', 'minotaur': 'Minotauro', 'manticore': 'Mantícora', 'erinys': 'Erinia', 'ladon' : 'ladon'
    };

    const FESTIVALES = {
        'party': 'Festival de la ciudad', 'games': 'Juegos Olímpicos',
        'theater': 'Representaciones teatrales', 'triumph': 'Desfile triunfal'
    };
    ...
    function criarPanelPrincipal() {
        ...
        panel.innerHTML = `<strong>⚔️ V11.80</strong>`;
    ...
    function criarPanelEncaixe() {
        ...
        panel.innerHTML = `
            <div id="bot-drag-header" style="padding:8px;background:#2a2a3a;border-bottom:1px solid #333;border-radius:8px 8px 0 0;cursor:move;display:flex;justify-content:space-between;align-items:center;">
                <h3 style="margin:0;color:#60a5fa;font-size:13px;">⚔️ Bot V11.80 (Pro)</h3>
        ...
        document.addEventListener('mousemove', (e) => { if(!isDragging) return; panel.style.setProperty('left', `${initialLeft + e.clientX - startX}px`, 'important'); panel.style.setProperty('top', `${initialTop + e.clientY - startY}px`, 'important'); panel.style.setProperty('margin', '0', 'important'); });
    ...
    function simularClick(el) { if(!el)return; el.dispatchEvent(new MouseEvent('mousedown',{bubbles:true})); el.dispatchEvent(new MouseEvent('mouseup',{bubbles:true})); el.click(); }
